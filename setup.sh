...
#   ENABLED_PROVIDERS, ONLINESIM_API_KEY, FIVESIM_API_KEY, PROVIDERS_DISPLAY
...
configure_env() {
  ...
  # Providers (optional)
  read -r -p "Enter ENABLED_PROVIDERS (e.g., onlinesim or onlinesim,numberland) [onlinesim]: " ENABLED_PROVIDERS_INPUT
  read -r -p "Enter ONLINESIM_API_KEY (leave empty if unused): " ONLINESIM_API_KEY_INPUT
  read -r -p "Enter FIVESIM_API_KEY (leave empty if unused): " FIVESIM_API_KEY_INPUT
  read -r -p "Enter PROVIDERS_DISPLAY (e.g., onlinesim:OnlineSim|numberland:Numberland) [onlinesim:OnlineSim]: " PROVIDERS_DISPLAY_INPUT
  ...
  sed -i "s|^ENABLED_PROVIDERS=.*|ENABLED_PROVIDERS=${ENABLED_PROVIDERS_INPUT:-onlinesim}|" .env
  sed -i "s|^ONLINESIM_API_KEY=.*|ONLINESIM_API_KEY=${ONLINESIM_API_KEY_INPUT}|" .env
  sed -i "s|^FIVESIM_API_KEY=.*|FIVESIM_API_KEY=${FIVESIM_API_KEY_INPUT}|" .env
  sed -i "s|^PROVIDERS_DISPLAY=.*|PROVIDERS_DISPLAY=${PROVIDERS_DISPLAY_INPUT:-onlinesim:OnlineSim}|" .env
